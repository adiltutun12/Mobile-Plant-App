...
class NovaBiljkaActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        trefleDAO = TrefleDAO("QLqAgsRfB04UZNMtWLL4lUwrNjrhc0LJPc01_TE28X4")
        initializeViews()
        ...
        dodajBiljkuBtn.setOnClickListener {
            if (validateFields()) {
                lifecycleScope.launch {
                    val newPlant = createNewPlant()
                    val fixedPlant = trefleDAO.fixData(newPlant)
                    Log.d("NovaBiljkaActivity", "Ispravljena biljka: $fixedPlant")
                    PlantRepository.addPlant(fixedPlant)
    ...
    private fun validateFields(): Boolean {
        ...
        if (nazivET.text.isBlank() || nazivET.text.length !in 2..40) {
            nazivET.error = "Dužina teksta mora biti između 2 i 40 znakova"
            isValid = false
        }
        if (porodicaET.text.isBlank() || porodicaET.text.length !in 2..40) {
            porodicaET.error = "Dužina teksta mora biti između 2 i 40 znakova"
            isValid = false
        }
    ...
    // Funkcija koja provjerava da li je bilo šta označeno
    ...
    private fun createNewPlant(): Biljka {
        ...
        dodajJeloBtn.setOnClickListener {
            ...
            if (updatedJelo.isEmpty()) {
                ...
                if (existingIndex == -1 || existingIndex == position) {
                    // Ažuriranje postojećeg jela ako nije duplikat ili isto jelo
